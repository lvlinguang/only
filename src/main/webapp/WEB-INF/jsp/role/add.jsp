						...
						<div class="dlg-form2-Info clearfix">
							<div class="boxL fl group_options">
								<c:forEach var="item" items="${permissions}">
									...
									<ul class="listRow">
										<c:forEach var="subitem"
											...
											<li><a rel="${item.text}"
												id="permission_${subitem.id}" href="javascript:void(0)">${subitem.text}</a></li>
										</c:forEach>
									</ul>
								</c:forEach>

								<!-- @{ foreach (var g in ((List
								<Permissions>)ViewBag.Permissions).Select(r => new
								{ r.GroupName }).Distinct()) {
								<div class="tit">
								...
								<ul class="listRow">
									...
									<Permissions>)ViewBag.Permissions).Where(r =>
